...
def open_config_window(url_label):
    ...
    def start_fuzz_test():
        ...
        min_num = cfg_now["min_numbers"]
        min_sym = cfg_now["min_symbols"]
            ...
            for key in keys:
                for value in values:
                    ...
                    try:
                        ...
                        if status == 200:
                            text_output.insert(tk.END, line + ("  [FLAGGED]" if flagged else "") + "\n", "ok")
...
# (filter_only_ok, on_line_click, load_paths_from_file, start_checking, GUI setup)
...
def filter_only_ok():
    ...
    for line in all_text:
        if "OK" in line or "Starting" in line or "Base:" in line or "Fuzzing" in line or "FLAGGED" in line:
